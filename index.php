...
function processStyle($style){
    global $styles;
    $src = $style->attributes->getNamedItem('src');
    $media = $style->attributes->getNamedItem('media');
    if($src == NULL){
        $cData = findCData($style);
        if($cData != NULL){
            if(array_key_exists($media->value, $styles['int'])){
                array_push($styles['int'][$media->value], $cData->data);
            }else{
                $styles['int'][$media->value] = array($cData->data);
            }
        }
    }else{
        if(array_key_exists($media->value, $styles['ext'])){
            array_push($styles['ext'][$media->value], $src->value);
        }else{
            $styles['ext'][$media->value] = array($src->value);
        }
    }
}
